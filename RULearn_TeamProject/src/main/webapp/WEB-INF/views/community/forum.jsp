...
<body>
	...
	<section class="container mar-bot2 m-height">
		...
		<c:url var="commForumUrl" value="/community/forum" />
		<c:url var="commAddBoardUrl" value="/community/add"/>
		
		<!-- 게시판 검색 -->
		<!-- 게시판 검색 -->
		<div class="mar-top2 mar-bot2">
			<form action="${commForumUrl}" method="get">
				<div class="row g-1">
					<div class="flex">
		...
		<div class="flex mar-bot1">
			...
			<c:choose>
				<c:when test="${not empty  searchValue}">
					<button class="btn font1 as_view" id="as1" type="button" onclick="location.href='${commForumUrl}?search=${searchValue}&sort=latest'">• 최신순</button>
					<button class="btn font1 as_view" id="as2" type="button" onclick="location.href='${commForumUrl}?search=${searchValue}&sort=comment'">• 댓글많은순</button>
					<button class="btn font1 as_view" id="as3" type="button" onclick="location.href='${commForumUrl}?search=${searchValue}&sort=like'">• 좋아요순</button>
				</c:when>
				<c:otherwise>
					<button class="btn font1 as_view" id="as1" type="button" onclick="location.href='${commForumUrl}?sort=latest'">• 최신순</button>
					<button class="btn font1 as_view" id="as2" type="button" onclick="location.href='${commForumUrl}?sort=comment'">• 댓글많은순</button>
					<button class="btn font1 as_view" id="as3" type="button" onclick="location.href='${commForumUrl}?sort=like'">• 좋아요순</button>
				</c:otherwise>
			</c:choose> 
			</div>
		...
		<table class="table table-hover mar-bot3 bor-col">
			...
			<tbody>
				<c:choose>
				<c:when test="${not empty forumBoardList}">
					<c:forEach items="${forumBoardList}" var="forumBoard">
						<c:url var="commBoardDetailUrl" value="/community/detail">
							<!-- 게시판 상세 페이지로 넘어갈 때 RequestParam으로 게시판 id 넘겨주기 -->
							<c:param name="cb_bid">${forumBoard.cb_bid}</c:param>
						</c:url>
						<tr onclick="location.href='${commBoardDetailUrl}'">
							<c:if test="${forumBoard.cb_catid eq 1}">
								<td class="txt-center"><span class="comm-cat-qa">Q&A</span></td>
							</c:if>
							<c:if test="${forumBoard.cb_catid eq 2}">
								<td class="txt-center"><span class="comm-cat-forum">고민/자유</span></td>
							</c:if>
							<c:if test="${forumBoard.cb_catid eq 3}">
								<td class="txt-center"><span class="comm-cat-tip">노하우/팁</span></td>
							</c:if>
							<td>${forumBoard.cb_title}&nbsp;&nbsp;[${forumBoard.cb_commentNum}]</td>
							<td>${forumBoard.cb_nickName}</td>
							<td>${forumBoard.cb_date}</td>
							<td><i class="bi bi-heart-fill"></i> ${forumBoard.cb_like}</td>
...
function pageUrlMove(num) {
	...
	var page = "${commForumUrl}";
	
	var url = "";	
	
	if(sort === "" && search === ""){
		url = "${commForumUrl}?page=";		
	}else if(sort !== "" && search === ""){
