...
<body>
	...
	<section class="container mar-bot2 m-height">
		...
		<c:url var="commTipUrl" value="/community/tip" />
		<c:url var="commAddBoardUrl" value="/community/add"/>
		
		<!-- 게시판 검색 -->
		<div class="mar-top2 mar-bot2">
			<form action="${commTipUrl}" method="get">
				<div class="row g-1">
					<div class="flex">
		...
		<div class="flex mar-bot1">
			...
			<c:choose>
				<c:when test="${not empty  searchValue}">
					<button class="btn font1 as_view" id="as1" type="button" onclick="location.href='${commTipUrl}?search=${searchValue}&sort=latest'">• 최신순</button>
					<button class="btn font1 as_view" id="as2" type="button" onclick="location.href='${commTipUrl}?search=${searchValue}&sort=comment'">• 댓글많은순</button>
					<button class="btn font1 as_view" id="as3" type="button" onclick="location.href='${commTipUrl}?search=${searchValue}&sort=like'">• 좋아요순</button>
				</c:when>
				<c:otherwise>
					<button class="btn font1 as_view" id="as1" type="button" onclick="location.href='${commTipUrl}?sort=latest'">• 최신순</button>
					<button class="btn font1 as_view" id="as2" type="button" onclick="location.href='${commTipUrl}?sort=comment'">• 댓글많은순</button>
					<button class="btn font1 as_view" id="as3" type="button" onclick="location.href='${commTipUrl}?sort=like'">• 좋아요순</button>
				</c:otherwise>
			</c:choose> 
			</div>
		...
		<table class="table table-hover mar-bot3 bor-col">
			...
			<tbody>
				<c:choose>
				<c:when test="${not empty tipBoardList}">
					<c:forEach items="${tipBoardList}" var="tipBoard">
						...
						<tr onclick="location.href='${commBoardDetailUrl}'">
							<c:if test="${tipBoard.cb_catid eq 1}">
								<td class="txt-center"><span class="comm-cat-qa">Q&A</span></td>
							</c:if>
							<c:if test="${tipBoard.cb_catid eq 2}">
							...
							<td>${tipBoard.cb_title}&nbsp;&nbsp;[${tipBoard.cb_commentNum}]</td>
							<td>${tipBoard.cb_nickName}</td>
							<td>${tipBoard.cb_date}</td>
							<td><i class="bi bi-heart-fill"></i> ${tipBoard.cb_like}</td>
