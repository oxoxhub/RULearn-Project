		...
		<div class="flex mar-bot1">
			...
			<c:choose>
				<c:when test="${not empty  searchValue}">
					<button class="btn font1 as_view" id="as1" type="button" onclick="location.href='${commMainUrl}?search=${searchValue}&sort=latest'">• 최신순</button>
					<button class="btn font1 as_view" id="as2" type="button" onclick="location.href='${commMainUrl}?search=${searchValue}&sort=comment'">• 댓글많은순</button>
					<button class="btn font1 as_view" id="as3" type="button" onclick="location.href='${commMainUrl}?search=${searchValue}&sort=like'">• 좋아요순</button>
				</c:when>
				<c:otherwise>
					<button class="btn font1 as_view" id="as1" type="button" onclick="location.href='${commMainUrl}?sort=latest'">• 최신순</button>
					<button class="btn font1 as_view" id="as2" type="button" onclick="location.href='${commMainUrl}?sort=comment'">• 댓글많은순</button>
					<button class="btn font1 as_view" id="as3" type="button" onclick="location.href='${commMainUrl}?sort=like'">• 좋아요순</button>
				</c:otherwise>
			</c:choose> 
			</div>
		...
		<table class="table table-hover mar-bot3 bor-col">
			...
			<tbody>
				...
				<c:when test="${not empty totalBoardList}">
					<c:forEach items="${totalBoardList}" var="totalBoard">
						<c:url var="commBoardDetailUrl" value="/community/detail">
							<!-- 게시판 상세 페이지로 넘어갈 때 RequestParam으로 게시판 id 넘겨주기 -->
							<c:param name="cb_bid">${totalBoard.cb_bid}</c:param>
						</c:url>
						<tr onclick="location.href='${commBoardDetailUrl}'">
							<c:if test="${totalBoard.cb_catid eq 1}">
								<td class="txt-center"><span class="comm-cat-qa">Q&A</span></td>
							</c:if>
							<c:if test="${totalBoard.cb_catid eq 2}">
								<td class="txt-center"><span class="comm-cat-forum">고민/자유</span></td>
							</c:if>
							<c:if test="${totalBoard.cb_catid eq 3}">
								<td class="txt-center"><span class="comm-cat-tip">노하우/팁</span></td>
							</c:if>
							<td>${totalBoard.cb_title}&nbsp;&nbsp;[${totalBoard.cb_commentNum}]</td>
							<td>${totalBoard.cb_nickName}</td>
							<td>${totalBoard.cb_date}</td>
							<td><i class="bi bi-heart-fill"></i> ${totalBoard.cb_like}</td>
...
function pageUrlMove(num) {
	...
	var page = "${commMainUrl}";
	
	var url = "";	
	
	if(sort === "" && search === ""){
		url = "${commMainUrl}?page=";		
	}else if(sort !== "" && search === ""){
