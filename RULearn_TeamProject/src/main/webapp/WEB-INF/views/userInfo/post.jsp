		...
		<div class="info-box1">
			<div class="flex">
				...
				<div>
					...
					<div class="info-lenght">작성글 ${fn:length(originalDatas)}</div>	
				</div>
			</div>
			...
			<div>
				<ul class="pagination justify-content-center">
					<c:if test="${pageData.hasPrevPage()}">
						<li class="page-item">
							<a class="page-link" href="${infoUrl}/post?id=${userData.AC_ID}&menu=${menu}&page=${pageData.prevPageNumber}">«</a>
						</li>
					</c:if>
					<c:forEach items="${pageData.getPageNumberList(pageData.currentPageNumber - 2, pageData.currentPageNumber + 2)}" var="num">
						<li class="page-item ${pageData.currentPageNumber eq num ? 'active' : ''}">
							<a class="page-link" href="${infoUrl}/post?id=${userData.AC_ID}&menu=${menu}&page=${num}">${num}</a>
						</li>
					</c:forEach>
					<c:if test="${pageData.hasNextPage()}">
						<li class="page-item">
							<a class="page-link" href="${infoUrl}/post?id=${userData.AC_ID}&menu=${menu}&page=${pageData.nextPageNumber}">»</a>
						</li>
					</c:if>
				</ul>
			</div>
