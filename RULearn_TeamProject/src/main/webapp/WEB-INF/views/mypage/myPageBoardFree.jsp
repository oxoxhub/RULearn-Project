		...
		<div class="row">
			<div class="col-md-3">
				<div class="d-flex flex-column flex-shrink-0 p-3 bg-light">
					<c:url value="/mypage" var="mypageURL"/>
					 <ul class="nav nav_pills mb-auto px-3 bg-light d-flex flex-column side_text">
			...
			<div class="col_md_9 ms-3">
				...
				<div id="myTabContent" class="tab-content">
					<br>
					<c:if test="${not empty forumBoardList}">
						<c:forEach items="${forumBoardList}" var="freeBoard">
							<c:url var="commBoardDetailUrl" value="/community/detail">
								<!-- 게시판 상세 페이지로 넘어갈 때 RequestParam으로 게시판 id 넘겨주기 -->
								<c:param name="cb_bid">${freeBoard.cb_bid}</c:param>
							</c:url>
							<c:set var="origin_title" value="${freeBoard.cb_title}" />
							<c:set var="diff_title"
								value="${fn:substring(origin_title, 0, 10)}" />
							<div class="tab-pane active show" id="free" role="tabpanel"
								onclick="location.href='${commBoardDetailUrl}'">
								<ul class="list-group">
									<li
										class="list-group-item d-flex justify-content-between align-items-center">
										<span class="col-1 badge bg-primary"> 고민/자유 </span>
										<div class="inner col-5">${diff_title}</div>
										<div class="col-3">${freeBoard.cb_wid}</div>
										<div class="col-2">${freeBoard.cb_date}</div>
										<div class="col-1">♥${freeBoard.cb_like}</div>
									</li>
								</ul>
							</div>
						</c:forEach>
