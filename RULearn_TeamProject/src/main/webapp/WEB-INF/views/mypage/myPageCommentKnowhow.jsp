		...
		<div class="row">
			...
			<div class="col_md_9 ms-3">
				...
				<div id="myTabContent" class="tab-content">
					<br>
					<c:if test="${not empty knowhowCommentList}">
						<c:forEach items="${knowhowCommentList}" var="knowhowComment">
							<c:url var="commBoardDetailUrl" value="/community/detail">
								<!-- 게시판 상세 페이지로 넘어갈 때 RequestParam으로 게시판 id 넘겨주기 -->
								<c:param name="cb_bid">${knowhowComment.cc_bid}</c:param>
							</c:url>
							<c:set var="origin_title" value="${knowhowComment.cc_content}" />
							...
							<div class="tab-pane active show" id="QAndA" role="tabpanel"
								...
								<ul class="list-group">
									<li
										...
										<span class="col-1 badge bg-primary"> 노하우&팁 </span>
										<div class="inner col-5">${diff_title}</div>
										<div class="col-3">${knowhowComment.cc_wid}</div>
										<div class="col-2">${knowhowComment.cc_date}</div>
										<div class="col-1">♥${knowhowComment.cc_like}</div>
									</li>
								</ul>
							</div>
						</c:forEach>
