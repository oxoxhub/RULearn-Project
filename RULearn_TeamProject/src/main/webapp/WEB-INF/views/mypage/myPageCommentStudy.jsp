		...
		<div class="row">
			...
			<div class="col_md_9 ms-3">
				...
				<div id="myTabContent" class="tab-content">
					<br>
					<c:if test="${not empty studyCommentList}">
						<c:forEach items="${studyCommentList}" var="studyComment">
							<c:url var="studyBoardDetailUrl" value="/studyboard/detail">
								<c:param name="id">${studyComment.sc_bId}</c:param>
							</c:url>
							<c:set var="origin_title" value="${studyComment.sc_content}" />
							...
							<div class="tab-pane active show" id="QAndA" role="tabpanel"
								...
								<ul class="list-group">
									<li
										...
										<span class="col-1 badge bg-primary"> 스터디 </span>
										<div class="inner col-5">${diff_title}</div>
										<div class="col-3">${studyComment.sc_wId}</div>
										<div class="col-2">${studyComment.sc_date}</div>
										<div class="col-1">♥${studyComment.sc_like}</div>
									</li>
								</ul>
							</div>
						</c:forEach>
