		...
		<div class="row">
			...
			<div class="col_md_9 ms-3">
				...
				<div id="myTabContent" class="tab-content">
					<br>
					<c:if test="${not empty projectBoardList}">
						<c:forEach items="${projectBoardList}" var="projectBoard">
							<c:url var="studyBoardDetailUrl" value="/studyboard/detail">
								<c:param name="id">${projectBoard.sb_bId}</c:param>
							</c:url>
							<c:set var="origin_title" value="${projectBoard.sb_title}" />
							<c:set var="diff_title"
								value="${fn:substring(origin_title, 0, 10)}" />
							<div class="tab-pane active show" id="project" role="tabpanel"
								onclick="location.href='${studyBoardDetailUrl}'">
								<ul class="list-group">
									<li
										class="list-group-item d-flex justify-content-between align-items-center">
										<c:if test="${projectBoard.sb_catId eq 5}">
											<span class="col-1 badge bg-primary"> 프로젝트 </span>
											<div class="inner col-5">${diff_title}</div>
											<div class="col-3">${projectBoard.sb_wId}</div>
											<div class="col-2">${projectBoard.sb_date}</div>
											<div class="col-1">♥${projectBoard.sb_like}</div>
										</c:if>
									</li>
								</ul>
							</div>
