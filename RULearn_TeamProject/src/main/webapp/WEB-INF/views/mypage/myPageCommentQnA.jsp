		...
		<div class="row">
			...
			<div class="col_md_9 ms-3">
				...
				<div id="myTabContent" class="tab-content">
					<br>
					<c:if test="${not empty qaCommentList}">
						<c:forEach items="${qaCommentList}" var="qaComment">
							<c:url var="commBoardDetailUrl" value="/community/detail">
								<!-- 게시판 상세 페이지로 넘어갈 때 RequestParam으로 게시판 id 넘겨주기 -->
								<c:param name="cb_bid">${qaComment.cc_bid}</c:param>
							</c:url>
							<c:set var="origin_title" value="${qaComment.cc_content}" />
							...
							<div class="tab-pane active show" id="QAndA" role="tabpanel"
								...
								<ul class="list-group">
									<li
										...
										<span class="col-1 badge bg-primary"> Q&A </span>
										<div class="inner col-5">${diff_title}</div>
										<div class="col-3">${qaComment.cc_wid}</div>
										<div class="col-2">${qaComment.cc_date}</div>
										<div class="col-1">♥${qaComment.cc_like}</div>
									</li>
								</ul>
							</div>
						</c:forEach>
