			...
			<div class="col_md_9 ms-3">
				...
				<div id="myTabContent" class="tab-content">
					...
					<c:if test="${not empty freeCommentList}">
						<c:forEach items="${freeCommentList}" var="freeComment">
							<c:url var="commBoardDetailUrl" value="/community/detail">
								<!-- 게시판 상세 페이지로 넘어갈 때 RequestParam으로 게시판 id 넘겨주기 -->
								<c:param name="cb_bid">${freeComment.cc_bid}</c:param>
							</c:url>
							<c:set var="origin_title" value="${freeComment.cc_content}" />
							...
							<div class="tab-pane active show" id="free" role="tabpanel"
								...
								<ul class="list-group">
									<li
										...
										<span class="col-1 badge bg-primary"> 자유/고민 </span>
										<div class="inner col-5">${diff_title}</div>
										<div class="col-3">${freeComment.cc_wid}</div>
										<div class="col-2">${freeComment.cc_date}</div>
										<div class="col-1">♥${freeComment.cc_like}</div>
									</li>
								</ul>
							</div>
						</c:forEach>
