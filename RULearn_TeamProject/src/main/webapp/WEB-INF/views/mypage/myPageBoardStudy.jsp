				...
				<div id="myTabContent" class="tab-content">
					...
					<c:if test="${not empty studyBoardList}">
						<c:forEach items="${studyBoardList}" var="studyBoard">
							...
							<c:set var="origin_title" value="${studyBoard.sb_title}" />
							<c:set var="diff_title"
								value="${fn:substring(origin_title, 0, 10)}" />

							<div class="tab-pane active show" id="study" role="tabpanel"
								onclick="location.href='${studyBoardDetailUrl}'">
								<ul class="list-group">
									<li
										class="list-group-item d-flex justify-content-between align-items-center">
										<c:if test="${studyBoard.sb_catId eq 4}">
											<span class="badge bg-primary col-1"> 스터디 </span>
											<div class="inner col-5">${diff_title}</div>
											<div class="col-3">${studyBoard.sb_wId}</div>
											<div class="col-2">${studyBoard.sb_date}</div>
											<div class="col-1">♥${studyBoard.sb_like}</div>
										</c:if>
									</li>
								</ul>
							</div>
